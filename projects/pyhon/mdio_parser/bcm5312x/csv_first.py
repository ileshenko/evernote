...
# Turnaround TA "The next bit timea are used to avoid contention on the MDIO pin when a read
# operation is performed. When a write operation is being performed, 10 must be sent by the
# BCM53125M during these two bit times. When a read operation is being performed< MDIO pin of the
# BCM53125M must be put in a high-impedance state during these bit times. The external PHY drives
...
#different from standard: data read on falling clock ??? !!!
#	if int(status) == 7:
