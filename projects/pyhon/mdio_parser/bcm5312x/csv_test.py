...
output_file = open('mdio_out.txt', 'a')
...
def proc_preamble(ts, mdio):
	...
	if int(buf_len) == 0 :
		output_file.write("%-11.7f " % float(ts))
...
#	if int(prev_clk) == 0 and int(mdc) == 0:
#		return

#	if int(prev_clk) == 1 and int(mdc) == 1 and int(status) != 0 :
...
#	output_file.write("WW %s %d %d\n" % (ts, int(mdc), int(mdio)))

	if int(status) == 0 :
		proc_idle(ts, mdio)
	elif int(status) == 1:
#		output_file.write(" ==== Pr ==== %s" % ts)
		proc_preamble(ts, mdio)
	elif int(status) == 2 :
#		output_file.write(" ==== St ==== %s" % ts)
		proc_start(ts, mdio)
	elif int(status) == 3 :
		proc_cmd(ts, mdio)
	elif int(status) == 4 :
